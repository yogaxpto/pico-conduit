...
FIRMWARE_ELF    := target/$(EMBEDDED_TARGET)/release/pico-conduit
FIRMWARE_DBG    := target/$(EMBEDDED_TARGET)/debug/pico-conduit
FIRMWARE_UF2    := pico-conduit.uf2
...
help:
	...
	@printf '  test-client          pico-conduit-client crate tests\n'
...
test-client:
	cargo test -p pico-conduit-client --target $(HOST_TARGET)
